...
from DReyeVR_utils import find_ego_vehicle2
import math
import keyboard
import asyncio
import websockets
...
def detect_lane_departure(vehicle, min_departure_distance=10):
    """
...
def respawn_vehicle(vehicle, new_location, new_rotation=None, reset_physics=True):

    try:
        if not isinstance(new_location, carla.Location):
            new_location = carla.Location(*new_location)
        
        if new_rotation and not isinstance(new_rotation, carla.Rotation):
            new_rotation = carla.Rotation(*new_rotation)
        elif not new_rotation:
            new_rotation = vehicle.get_transform().rotation  # Keep current rotation
        
        # Disable physics during teleportation
        ...
        # Optional: Project to road surface
        waypoint = vehicle.get_world().get_map().get_waypoint(new_location, project_to_road=True)
        if waypoint:
            adjusted_transform = carla.Transform(
                waypoint.transform.location + carla.Location(z=0.5),
                new_rotation
            )
            vehicle.set_transform(adjusted_transform)
        
        # Restore physics if requested
        if reset_physics:
        ...
        print(f"Vehicle respawned at {new_location}")
        return True
        
    except Exception as e:
        print(f"Failed to respawn vehicle: {str(e)}")
...
class CarlaDataProvider:
    def __init__(self):
        ...
        self.destination = carla.Location(x=-228.227371, y=325.148773, z=0.001746)
        self.last_data = None
        ...
        self.route_signs = [
                {
                'location': carla.Location(x=-61.958035, y=-97.475723, z=0.001768),
                'name': "distance-to-empty full"
            },
            {
                'location': carla.Location(x=52.236748, y=407.193756, z=0.001763),
                'name': "fog-light"
            },
            {
                'location': carla.Location(x=-190.572922, y=319.786407, z=0.001740),
                'name': "tpms-warning"
            },
        ]
        self.speed_points = [
            {
                'location': carla.Location(x=421.525696, y=354.069794, z=0.001683),
                'name': "60"
            },
            {
                'location': carla.Location(x=443.029785, y=353.852234, z=0.001702),
                'name': "30"
            },
            {
                'location': carla.Location(x=237.405167, y=142.779388, z=0.011178),
                'name': "30"
            },
            {
                'location': carla.Location(x=460.948151, y=20.493486, z=0.001731),
                'name': "60"
            },
            {
                'location': carla.Location(x=359.298309, y=20.512848, z=0.001746),
                'name': "50"
            },
            {
                'location': carla.Location(x=14.026157, y=-52.135422, z=0.001653),
                'name': "90" #highway
            },
            {
                'location': carla.Location(x=-61.958035, y=-97.475723, z=0.001768),
            ...
            {
                'location': carla.Location(x=51.350410, y=113.011314, z=0.001665),
                'name': "50" #exit highway
            },
            {
                'location': carla.Location(x=-165.268417, y=116.440407, z=0.001699),
                'name': "60"
            },
            {
                'location': carla.Location(x=-205.276245, y=127.874352, z=0.001708),
                'name': "30"
            },
            {
                'location': carla.Location(x=-228.761963, y=213.564148, z=0.001709),
                'name': "30"
            },
        ]
        self.route_points = [
            # {
            #     'location': carla.Location(x=461, y=353, z=0),
            #     'turn': 'straight',
            #     'name': "Start"
            # },
            {
                'location': carla.Location(x=469, y=353, z=0),
                'turn': 'left',
                'name': "FirstTurn"
            },
            {
                'location': carla.Location(x=482, y=232, z=0),
                'turn': 'left',
                'name': "SecondTurn"
            },
            {
                'location': carla.Location(x=250, y=218, z=0),
                'turn': 'right',
                'name': "ThirdTurn"
            },
             {
                'location': carla.Location(x=237, y=87, z=0),
                'turn': 'right',
                'name': "FourthTurn"
            },
             {
                'location': carla.Location(x=473, y=82, z=0),
                'turn': 'left',
                'name': "fifthTurn"
            },
             {
                'location': carla.Location(x=484, y=33, z=0),
                'turn': 'left',
                'name': "sixthTurn"
            },
             {
                'location': carla.Location(x=355, y=21, z=0),
                'turn': 'right',
                'name': "seventhTurn"
            },
            {
                'location': carla.Location(x=50, y=121, z=0),
                'turn': 'right',
                'name': "eigthTurn"
            },
            {
            ...
            {
                'location': carla.Location(x=-239.306564, y=204.103958, z=0.001666),#Location(x=-104.356354, y=214.890228, z=0.001703),
                'turn': 'left',
                'name': "eleventhTurn"
            },
            {
                'location': carla.Location(x=-104.356354, y=214.890228, z=0.001703),
                'turn': 'right',
                'name': "eleventhTurn"
            },
            {
                'location': carla.Location(x=-100.839722, y=254.235214, z=0.001707),
                'turn': 'right',
                'name': "twelveTurn"
            },
            {
                'location': carla.Location(x=-178.721146, y=259.227203, z=0.001740),
                'turn': 'left',
                'name': "thirteenthTurn"
            },
            {
            ...
            {
                'location': carla.Location(x=-228.227371, y=325.148773, z=0.001746),
                'name': "Finish"
            }
        ]
    ...
    async def connect_to_carla(self, host='localhost', port=2000):
        ...
        try:
            ...
            success = respawn_vehicle(self.vehicle, carla.Location(x=305.348236, y=354.800262, z=0.001752))
            # self.vehicle = find_ego_vehicle2(self.world, carla.Location(x=-228.227371, y=325.148773, z=0.001746))
    ...
    def get_vehicle_data(self):
        ...
        try:
            ...
            #     voice_alert("key A is pressed")
            # if keyboard.is_pressed('b'):
            #     voice_alert("key B is pressed")
            # if keyboard.is_pressed('c'):
            #     voice_alert("key C is pressed")
            ...
            if progress_signs:
                if progress_signs['status'] == 'enroute':
                    if progress_signs['distance'] < 10: 
                        if progress_signs['name'] == "distance-to-empty full":
                            self.shared_data["distance_to_empty"] = "full"
                            voice_alert("Check distance travelled")
                        if progress_signs['name'] == "fog-light":
                            self.shared_data["fog_light"] = True
                            voice_alert("Check fog light")
                        if progress_signs['name'] == "tpms-warning":
                            self.shared_data["tpms"] = True 
                            voice_alert("Check TPMS") 
            # Update data
