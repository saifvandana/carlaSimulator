...
class TrafficLightController:
    ...
    def __init__(self, lights):
        ...
        # Do NOT freeze — freezing affects the whole junction group
        # Instead we call set_state() every tick to override CARLA's controller
        # just for our two lights
        for tl in self.lights:
            tl.set_state(carla.TrafficLightState.Red)
        print(f"[TLController] Controlling {len(self.lights)} lights — starting RED")
        print(f"[TLController] Other junction lights remain on CARLA's controller")
 
    def update(self, t):
        # Cycle order: GREEN -> YELLOW -> RED -> GREEN -> ...
        pos = (t + CYCLE_OFFSET) % CYCLE_TOTAL
        if pos < GREEN_DURATION:
            self.phase = self.PHASE_GREEN
            state      = carla.TrafficLightState.Green
        elif pos < GREEN_DURATION + YELLOW_DURATION:
    ...
    def time_remaining(self, t):
        pos = (t + CYCLE_OFFSET) % CYCLE_TOTAL
        if pos < GREEN_DURATION:
            return GREEN_DURATION - pos
        elif pos < GREEN_DURATION + YELLOW_DURATION:
            return (GREEN_DURATION + YELLOW_DURATION) - pos
        else:
...
class QueueController:
    ...
    def update(self, phase, t):
        ...
        # Q2 and Q3 creep continues through YELLOW 1 and GREEN 1
        # until they reach their targets
        if self._yellow_count >= 1 and not self._q2_departed:
            if phase in ("YELLOW", "GREEN") and self._yellow_count == 1:
                if not self._q2_creep1_done:
                    self._q2_creep1_done = self._creep_toward(
                        self.vehicles[1], Q1_SPOT
                    )
                elif phase == "GREEN":
                    self._hold(self.vehicles[1])
 
        if self._yellow_count >= 1 and not self._q3_departed:
            if phase in ("YELLOW", "GREEN") and self._yellow_count == 1:
                if not self._q3_creep1_done:
                    self._q3_creep1_done = self._creep_toward(
                        self.vehicles[2], Q2_SPOT
                    )
                elif phase == "GREEN":
        ...
        # Q3 creeps to Q1 spot through YELLOW 2 and GREEN 2
        if self._yellow_count >= 2 and not self._q3_departed:
            if phase in ("YELLOW", "GREEN") and self._yellow_count == 2:
                if not self._q3_creep2_done:
                    self._q3_creep2_done = self._creep_toward(
                        self.vehicles[2], Q1_SPOT
                    )
                elif phase == "GREEN" and self._q3_creep2_done:
                    self._hold(self.vehicles[2])
        ...
        if self._q1_departed and self.vehicles[0] and self.vehicles[0].is_alive:
            loc = self.vehicles[0].get_location()
            if loc.x > 50.0:   # well past intersection — destroy
                self.vehicles[0].destroy()
            else:
                self.drivers[0].run_step()
 
        if self._q2_departed and self.vehicles[1] and self.vehicles[1].is_alive:
            loc = self.vehicles[1].get_location()
            if loc.x > 50.0:
                self.vehicles[1].destroy()
            else:
                self.drivers[1].run_step()
 
        if self._q3_departed and self.vehicles[2] and self.vehicles[2].is_alive:
            loc = self.vehicles[2].get_location()
            if loc.y < -110.0:   # well past right turn — destroy
                self.vehicles[2].destroy()
            else:
