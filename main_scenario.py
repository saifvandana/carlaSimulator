        ...
        if self.state == "waiting":
            if t >= self._spawn_t:
                ...
                if npc:
                    ...
                    print(f"[{self.label}] Spawned successfully  id={npc.id}")
                # else: silently retries every tick until it succeeds
                #       (no log spam, but check terminal for [Spawn] FAILED lines)
            return f"{self.label} — waiting to spawn"
...
class CongestionCutIn:
    ...
    def _update(self, world, ego, t):
        ...
        if self.state == "overtake":
            ...
            spd = SPEED_CRUISE + progress*(SPEED_CONG_MAX-SPEED_CRUISE)
            self.ctrl.tick(spd)
...
def main():
    ...
    try:
        ...
        # Keep ego at its original spawn location, just switch lane
        # from -4 to -2 (A1-A4 events expect ego in lane -2)
        cur_loc = ego.get_transform().location
        cur_wp  = world.get_map().get_waypoint(
            cur_loc, project_to_road=True,
            lane_type=carla.LaneType.Driving)

        if cur_wp:
            lane2_wp = wp_in_lane(cur_wp, -2)
            if lane2_wp:
                tf = carla.Transform(
                    carla.Location(x=lane2_wp.transform.location.x,
                                   y=lane2_wp.transform.location.y,
                                   z=lane2_wp.transform.location.z + 0.3),
                    lane2_wp.transform.rotation)
                ego.set_simulate_physics(False)
                ego.set_transform(tf)
                time.sleep(0.1)
                ego.set_simulate_physics(True)
                print(f"[Ego] Switched to lane -2 at original spawn: "
                      f"road={lane2_wp.road_id}  "
                      f"x={tf.location.x:.1f}  y={tf.location.y:.1f}")
            else:
                print("[Ego] Could not find lane -2 — staying at current lane")
        else:
            print("[Ego] Could not get waypoint — staying at spawn location")
        ...
        # Queue head positioned at the exit location near x=-353.91, y=16.01
        # (road 1401, lane -3). Vehicles spawn going BACKWARD from there
        # so the queue forms naturally leading up to the exit.
        # Exit found 10m behind ego spawn: road 43 -> road 275 (junction, 1 lane)
        # Anchor queue just before this junction, still on road 43 lane -4
        CONGESTION_EXIT_LOCATION = carla.Location(x=220.2, y=-364.2, z=0.23)

        print("[Congestion] Pre-spawning queue ending at exit location "
              f"x={CONGESTION_EXIT_LOCATION.x:.1f} y={CONGESTION_EXIT_LOCATION.y:.1f}...")

        exit_wp = world.get_map().get_waypoint(
            CONGESTION_EXIT_LOCATION, project_to_road=True,
            lane_type=carla.LaneType.Driving)

        if exit_wp:
            print(f"[Congestion] Raw exit_wp: lane={exit_wp.lane_id}  "
                  f"x={exit_wp.transform.location.x:.1f}  "
                  f"y={exit_wp.transform.location.y:.1f}  "
                  f"road={exit_wp.road_id}")

            # Make sure we're in lane -4 (exit lane) at the exit
            lane4_exit_wp = wp_in_lane(exit_wp, -4)
            if lane4_exit_wp is None:
                print("[Congestion] WARNING: could not walk to lane -4, "
                      "using raw exit_wp as-is")
                lane4_exit_wp = exit_wp
            else:
                print(f"[Congestion] Walked to lane -4: "
                      f"x={lane4_exit_wp.transform.location.x:.1f}  "
                      f"y={lane4_exit_wp.transform.location.y:.1f}")

            # Build the queue by walking AWAY from the exit, toward where
            # ego approaches from. Whichever direction (next/previous)
            # moves AWAY from the exit determines vehicle ordering:
            # vehicle 0 must be the FRONT of the queue = closest to exit.
            # We test both directions and pick the one whose distance
            # from the exit location increases (i.e. moves away).
            exit_loc = lane4_exit_wp.transform.location

            def dist_from_exit(wp_):
                d = wp_.transform.location
                return ((d.x-exit_loc.x)**2 + (d.y-exit_loc.y)**2) ** 0.5

            test_next = lane4_exit_wp.next(12.0)
            test_prev = lane4_exit_wp.previous(12.0)

            walk_away_func = lambda wp_: wp_.previous(12.0) #None
            # if test_next and dist_from_exit(test_next[0]) > 1.0:
            #     walk_away_func = lambda wp_: wp_.next(12.0)
            #     print("[Congestion] Walking AWAY from exit using next()")
            # elif test_prev and dist_from_exit(test_prev[0]) > 1.0:
            #     walk_away_func = lambda wp_: wp_.previous(12.0)
            #     print("[Congestion] Walking AWAY from exit using previous()")
            # else:
            #     walk_away_func = lambda wp_: wp_.previous(12.0)
            #     print("[Congestion] WARNING: could not verify direction, "
            #           "defaulting to previous()")

            # Vehicle 0 = 2m TOWARD the exit (slightly past/into it,
            # front of queue right at the junction threshold)
            toward_exit = walk_away_func.__name__   # not reliable, use direct test
            # Determine the "toward exit" direction (opposite of walk_away_func)
            if test_next and dist_from_exit(test_next[0]) > 1.0:
                # next() moves away, so previous() moves toward exit
                toward = lane4_exit_wp.previous(25.0)
            else:
                # previous() moves away, so next() moves toward exit
                toward = lane4_exit_wp.next(2.0)
            cur_wp = toward[0] if toward else lane4_exit_wp
            print(f"[Congestion] Vehicle 0 placed 2m toward exit: "
            ...
            for i in range(NUM_CONG_VEHS):
                ...
                print(f"[Congestion] Spawning vehicle {i}: "
                      ...
                      f"x={cur_wp.transform.location.x:.1f}  "
                      f"y={cur_wp.transform.location.y:.1f}  "
                      f"dist_from_exit={dist_from_exit(cur_wp):.1f}m")
                ...
                if npc:
                    ...
                    all_npcs.append(npc)
                # Step AWAY from exit for the next vehicle (further back in queue)
                nxt = walk_away_func(cur_wp)
                cur_wp = nxt[0] if nxt else None
        print(f"[Congestion] {len(congestion_npcs)} vehicles pre-spawned, "
              f"queue ending at exit location")
        congestion_spawned = True   # already done
        ...
        while True:
            ...
            # ── Congestion queue already spawned at t=0 — nothing here ──
            # (spawned far ahead at scenario start, see below)
            pass
