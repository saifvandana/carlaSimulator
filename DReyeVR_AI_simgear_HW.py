...
def main():
    ...
    try:
        ...
        wayDirections = {208:"go to left lane and continue straight",18:"Change lane to the left", 317:"Continue Straight", 347:"keep in the left most lane", 
                         157:"take the next exit", 220:"continue straight and merge on the highway", 365:"Continue Straight", 334:"take the next exit",
                         306:"merge on the highway", 110:"move to the left lane", 86:"Continue Straight"}
                        # 55:"take the next exit", 337:"merge on the next highway", 269:"move to the left lane", 132:"Continue straight",
                        # 334:"take the next exit", 306:"merge on the next highway", 292:"change to left lane and go straight"}
        wayD = deepcopy(wayDirections)
        while True:
            root.update()
            world.wait_for_tick()
            # print(ego_vehicle.get_location())w
            if bool(wayD):
                ways = compute_distance(world,ego_vehicle,list(wayD.keys())[0])
                dist_to_dest = len(ways)
                egoLoc = ego_vehicle.get_location()
                # draw_ways(ways[:6],world)5
                if(dist_to_dest<15):
