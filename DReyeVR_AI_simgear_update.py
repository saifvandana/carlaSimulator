...
def main():
    ...
    root.geometry("300x150+1200+830")
    # root.mainloop()
    try:
        ...
        wayDirections = {0:"take next right",85:"Continue straight", 9:"Take the next left", 86:"Continue straight", 166:"Take the next right",
                         ...
                         186:"Take the next left", 205:"Keep straight", 143:"Take two left turns", 153:"keep straight", 
                         93:"take next left", 76:"take next right", 141:"keep straight", 223:"take right", 89:"keep straight",
        ...
        while True:
            ...
            if bool(wayDirections):
                ways = compute_distance(world,ego_vehicle,list(wayDirections.keys())[0])
                dist_to_dest = len(ways)
                egoLoc = ego_vehicle.get_location()
                # draw_ways(ways[:6],world)
                if(dist_to_dest<15):
                    first_key = next(iter(wayDirections))
                    first_value = wayDirections.pop(first_key)
