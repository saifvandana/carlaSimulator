...
def websocket_sender(shared_data, websocket_url):
    ws = create_connection(websocket_url)
    try:
        while True:
            if shared_data.get("updated"):
                rpm = shared_data["rpm"]
                speed_kmph = shared_data["speed_kmph"]
                message = f'{{"rpm": {rpm}, "speed": {speed_kmph}}}'
                ws.send(message)
...
def main():
    ...
    websocket_url = "ws://127.0.0.1:7000" 
    shared_data = {"rpm": 0, "speed_kmph": 0, "updated": False}
    websocket_thread = threading.Thread(target=websocket_sender, args=(shared_data, websocket_url))
    ...
    try:
        ...
        while True:
            ...
            speed_kmph = round(speed * 3.6, 2)
            rpm = calculate_rpm(speed_kmph, wheelR)

            # Update data
            shared_data["rpm"] = rpm
            shared_data["speed_kmph"] = speed_kmph
