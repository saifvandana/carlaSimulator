...
def set_DReyeVR_autopilot(world, traffic_manager):
    ...
    if DReyeVR_vehicle is not None:
        # Here we disable autopilot so you (or your script) can override controls.
        DReyeVR_vehicle.set_autopilot(True, traffic_manager.get_port())
        print("Successfully set autopilot on on ego vehicle")
    return DReyeVR_vehicle

def set_DReyeVR_agent(world, traffic_manager):
    DReyeVR_vehicle = find_ego_vehicle(world)
    if DReyeVR_vehicle is not None:
        print('0')
        agent = BasicAgent(DReyeVR_vehicle)
        print('1')
        spawn_ps = world.get_map().get_spawn_points()
        destination = spawn_ps[3].location
        agent.set_destination(destination)
        agent.set_target_speed(80)
    return agent , DReyeVR_vehicle
...
def main():
    ...
    try:
        ...
        # Retrieve the ego vehicle (or find it) and disable its autopilot so we can control it.
        
        
        
        # ego_vehicle = set_DReyeVR_autopilot(world, traffic_manager)
        agent , ego_vehicle= set_DReyeVR_agent(world, traffic_manager)

        if ego_vehicle is not None:
            print("ego vehicle found in the world!")
            return
        ...
        if ego_vehicle is None:
            print("No ego vehicle found in the world!")
            return
