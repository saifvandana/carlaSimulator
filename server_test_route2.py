...
class CarlaDataProvider:
    def __init__(self):
        ...
        self.route_signs = [
                {
                'location': carla.Location(x=-50, y=214, z=0),
                'name': "distance-to-empty low"
            },
            {
                'location': carla.Location(x=270.534302, y=382.413422, z=0.077940),
                'name': "construction"
            },
            {
                'location': carla.Location(x=489, y=20, z=0),
                'name': "hazard"
            },
            {
                'location': carla.Location(x=146.256027, y=328.753082, z=0.001642),
                'name': "fuel-warning"
            },
        ]
        self.speed_points = [
            {
                'location': carla.Location(x=-235, y=220, z=0),
                'name': "30"
            },
            {
                'location': carla.Location(x=-42, y=255, z=0),
                'name': "60"
            },
            {
                'location': carla.Location(x=-182, y=329, z=0),
                'name': "60"
            },
            {
                'location': carla.Location(x=-152, y=332, z=0),
                'name': "50"
            },
            {
                'location': carla.Location(x=-272.174652, y=496.958466, z=0.001683),
                'name': "90" #highway
            },
            {
                'location': carla.Location(x=55.745964, y=464.688934, z=0.001677),
                'name': "50"
            },
            {
                'location': carla.Location(x=284.018311, y=370.978119, z=0.001690),
                'name': "60"
            },
            {
                'location': carla.Location(x=540.411743, y=328.880219, z=0.001693),
                'name': "90"
            },
            {
                'location': carla.Location(x=540.883301, y=75.152901, z=0.001687),
                'name': "60"
            },
            {
                'location': carla.Location(x=540.805786, y=59.190224, z=0.001654),
                'name': "30"
            },
            {
                'location': carla.Location(x=515.533081, y=21.358416, z=0.001690),
                'name': "30"
            },
            {
                'location': carla.Location(x=209.383041, y=348.645538, z=0.001605),
                'name': "30"
            },
            {
                'location': carla.Location(x=146.256027, y=328.753082, z=0.001642),
             ...
             {
                'location': carla.Location(x=146.714096, y=154.433655, z=0.001668),
                'name': "30"
            },
        ]
        self.route_points = [
            # {
            #     'location': carla.Location(x=235, y=317, z=0),
            #     'turn': 'straight',
            #     'name': "Start"
            # },
            {
            ...
            },
            {
             ...
             {
                'location': carla.Location(x=-177, y=260, z=0),
                'turn': 'left',
                'name': "FourthTurn"
            },
             {
                'location': carla.Location(x=-189, y=314, z=0),
                'turn': 'left',
                'name': "fifthTurn"
            },
             {
             ...
             {
                'location': carla.Location(x=55, y=465, z=0),
                'turn': 'right',
                'name': "seventhTurn"#exit highway
            },
            {
                'location': carla.Location(x=529.012573, y=353.361267, z=0.001655),
                'turn': 'left',
                'name': "eigthTurn"
            },
            {
                'location': carla.Location(x=537.247681, y=27.300755, z=0.001653),
                'turn': 'left',
                'name': "eigthTurn"
            },
            {
            ...
            {
                'location': carla.Location(x=479, y=207, z=0),
                'turn': 'right',
                'name': "ninthTurn"
            },
            {
                'location': carla.Location(x=245, y=218, z=0),
                'turn': 'left',
                'name': "tenthTurn"
            },
            {
                'location': carla.Location(x=233, y=341, z=0),
                'turn': 'right',
                'name': "eleventhTurn"
            },
            {
    ...
    async def connect_to_carla(self, host='localhost', port=2000):
        ...
        try:
            ...
            success = precise_respawn(self.vehicle, carla.Location(x=-235.227371, y=320.148773, z=0.001746), yaw=None)
            ...
            if progress_signs:
                if progress_signs['status'] == 'enroute':
                    if progress_signs['distance'] < 10: 
                        if progress_signs['name'] == "distance-to-empty low":
                            self.shared_data["distance_to_empty"] = "low"
                            voice_alert("Check distance travelled")
                        if progress_signs['name'] == "construction":
                            self.shared_data["construction_distance"] = int(progress_signs['distance'])
                            voice_alert("Check construction sign")
                        if progress_signs['name'] == "hazard":
                            self.shared_data["construction_distance"] = ""
                            self.shared_data["hazard_indicator"] = True
                            voice_alert("Check hazard indicator")
                        if progress_signs['name'] == "fuel-warning":
                            self.shared_data["fuel_warning"] = True
                            voice_alert("Check fuel warning")
...
# async def carla_data_handler(websocket):
#     """Handle WebSocket connections and send CARLA data"""
#     logger.info(f"New client connected: {websocket.remote_address}")
#     # Connect to CARLA
#     carla_provider = CarlaDataProvider()
#     connected = await carla_provider.connect_to_carla()
...
#     if not connected:
#         await websocket.send(json.dumps({"error": "Failed to connect to CARLA"}))
#         return
